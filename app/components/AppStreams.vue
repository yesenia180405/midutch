...
const streamList = computed(() => props.streams || [])
</script>

<template>
  <section class="pb-8 text-white flex flex-col">
    <h2 class="pt-5 pl-2 font-semibold sm:text-base md:text-lg">
      <span class="text-main-text">Live channels</span> we think you'll like
    </h2>

    <section class="grid grid-cols-2 md:grid-cols-4 px-3 gap-8 md:gap-14 py-6 pr-7">
      <div
        v-for="stream in showMore ? streamList.slice(props.startSlice, (props.endSlice + 4)) : streamList.slice(props.startSlice ?? 0, props.endSlice ?? 3)"
        :key="stream.id">
        <AppCardStream
          :id="stream.id"
